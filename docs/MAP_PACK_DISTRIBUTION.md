...
`pin-release`의 파일 목록은 서버 자산의 이름·크기·SHA-256을 재고표와 대조한 뒤 확정한 `<SHA-256>.pmtiles` 이름을 한 줄씩 담는다. 같은 팩을 다른 릴리스로 다시 고정하려 하면 실패한다. `install-pinned`는 팩마다 고정한 서로 다른 릴리스 주소를 사용한다. 원격 설치는 릴리스가 **게시된 뒤에만** 가능하다. 현재 일곱 릴리스는 초안이라 공개 다운로드에 사용할 수 없다. 오프라인 지도 런타임은 네트워크 요청을 하지 않는다. 원격 설치 명령은 사용자가 별도로 실행하는 설치·갱신 단계용이다.

...
| S2 Manifest | 승인 3개 레이어 → SHA·버전 잠금 | S1 | 입력 재현 가능 | `data/sources.toml` | PASS_ROADS_DISTRICTS |
| S3 Adapter | 나주 도로·SGIS 경계 → canonical 후보 | S2 | raw 필드가 builder로 새지 않음 | source checksum·형상 검사 | PASS_ROADS_DISTRICTS |
| S4 Schema | 후보 → WGS84 f64 feature | S3 | Mappa ID ≠ 원본 gid/ADM_CD | GeoDB roundtrip | PASS_ROADS_DISTRICTS |
...
| S10 Provenance | feature → lineage | S8 | runtime 타일에서 제외 | 8,318건 추적·decode | PARTIAL: revision 체인 없음 |
| S11 Tiles | GeoDB → PMTiles | S8,S9 | raw/OSM/NE 미입력 | 79 tiles, 모든 줌 decode | PASS_ROADS_DISTRICTS |
| S12 Renderer | PMTiles → Mac 화면 | S11 | 로컬 한 파일만 열기 | z14·z15 Metal capture, 송월동 지명 확인 | PASS_ROADS_DISTRICTS |

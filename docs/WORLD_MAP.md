...
상세 원천 확대는 [모나코·Queens 건물](WORLD_BUILDINGS_PROGRESS.md), [뉴욕주 62개 카운티 도로](NY_STATE_ROADS_PROGRESS.md), [뉴저지주 21개 카운티 도로](NJ_STATE_ROADS_PROGRESS.md), [GB 공식 RoadLink 52개 격자](GB_ROADS_PROGRESS.md), [캐나다 6개 지역 NRN 도로](CA_ROADS_PROGRESS.md), [뉴욕시 5개 카운티 수면](WORLD_WATER_PROGRESS.md), [뉴욕시 주변 공원 경계](WORLD_PARKS_PROGRESS.md)에 실증했다. 기본 세계 모드는 기존 6개, GB 52개, 캐나다 6개, 총 64개 지역 패키지를 필요할 때 결합한다. 이 지역 밖의 상세 지리는 구축되지 않았다.
사용자가 선택한 기존 공유조건 금지 규칙에 따라 도로는 국가별 공식 원천으로 진행한다. 미국 뉴욕주·뉴저지주 83개 카운티, Great Britain의 OS Open Roads RoadLink, 캐나다 6개 지역의 NRN Road Segment 원천을 처리했으며 세계 상세 도로망은 아직 없다.
...
| z10–z15 | 캐나다 PE·NS·NB·NT·NU·YT | 공식 NRN Road Segment 지역 PMTiles | 다른 7개 주 제외; 연결성·현장 위치 정확도 미검증 |
...
### 캐나다 6개 지역 도로 확장 — 2026-09-24

[캐나다 NRN 원천 감사](CA_ROADS_PROGRESS.md)에서 PE·NS·NB·NT·NU·YT의 `ROADSEGID`가 각 지역 안에서 고유한 도로선 합계 230,326개를 채택하고 거절 0개를 기록했다. 여섯 지역 PMTiles의 실제 타일 합계 192,910개를 전부 해독했고 오류는 0개였다. Charlottetown·Halifax·Fredericton·Yellowknife·Iqaluit·Whitehorse z14 기본 화면은 각각 타일 실패 0이고 캐나다 출처 표기가 보인다. NAD83(CSRS)·WGS84 독립 위치 비교, 지역 경계 연결성, 나머지 7개 주 구축은 아직 없다.
...
이 감사는 파일 해독과 표시 확인이다. 실제 현장 정확도, 도로·수계 완전성, 국가별 건물 커버리지의 독립 검증은 아니다. 원본 선이 없는 지역을 새 선으로 채우지 않는다. 기존 50m와 동아시아 10m 파일은 비교·회귀용으로 보관한다. 기본 세계 모드는 세계 110m·10m 파일을 열고 기존 6개·GB 52개·캐나다 6개, 총 64개 승인 지역 PMTiles를 필요할 때 연다. 목록의 출처 manifest는 시작 시 비공유조건 권리 판정을 통과하며, 아카이브의 지리 범위·출처 표기는 첫 사용 시 대조한다.

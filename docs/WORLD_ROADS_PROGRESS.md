...
| 미국 | Census 2025의 **24개 주·DC, 1,397개 카운티 상당 원천 단위**를 실증. [세계 지도 범위 표](WORLD_MAP.md)와 각 주의 검증 기록에 채택·제외·타일 수를 남겼다. 최근 [플로리다주 67개 카운티 도로](FL_STATE_PROGRESS.md)의 원본 684,796행 계보와 132,265개 타일을 감사했다. 다른 주 카운티는 미수집·미검증. 공식 배포 단위는 카운티 상당 단위 All Roads 파일. |
...
| 호주 서부 | [Main Roads WA Road Network](AU_WA_ROADS_PROGRESS.md): 공식 CC BY 4.0 ZIP에서 차량 도로 188,864개를 채택, 1,024개 비차량·계획 선형 분리. 9개 지역 팩 253,612개 타일 해독, 같은 팩 내부 확정 경계 불일치 0. 도로망 완전성·독립 위치 정확도·지역 사이 연결성 미검증. |
| 호주 빅토리아 | [DTP Managed Roads](AU_VIC_DTP_ROADS_PROGRESS.md): 공식 CC BY 4.0 GeoJSON에서 DTP 관리 도로 90,607개 채택, 미해석 코드 190개 분리. 47,604개 타일 해독, 팩 내부 확정 경계 불일치 0. 주 전체 도로·독립 위치 정확도·연결성 미검증. |
| 호주 퀸즐랜드 | [Queensland Roads and Tracks](AU_QLD_QRT_ROADS_PROGRESS.md): 공식 CC BY 4.0 선형 562,728개 중 일반 차량 도로 411,847개 채택, 150,881개 비차량·제한·비운영 선형 분리. 79개 지역 팩 303,681개 타일 해독, 팩 내부 확정 경계 불일치 0. 트랙·접근 제한 도로 별도, 지역 경계 연결성·독립 위치 정확도·Metal 화면·iPhone 성능 미검증. |

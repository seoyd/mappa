...
사용자 결정에 따라 **ODbL 원천을 현재 Mappa canonical GeoDB에 넣지 않는다.** 전 세계 도로의 국가별 공식·허용형 원천을 찾고, 같은 출처·품질 게이트로 지역별로 구축한다. 첫 실증인 미국 Census 뉴욕 카운티 한 파일을 뉴욕시 5개 카운티로, 이어 [뉴욕주 62개 카운티](NY_STATE_ROADS_PROGRESS.md)로 확장하고 [뉴저지주 21개 카운티](NJ_STATE_ROADS_PROGRESS.md)를 추가했다. 현재 기본 세계 모드는 두 주와 [Great Britain TQ 격자](GB_ROADS_PROGRESS.md)의 도로 팩을 사용한다. **전 세계 상세 도로망은 아직 없다.**
...
| Great Britain | [OS Open Roads 실증](GB_ROADS_PROGRESS.md): 무료 OGL v3, 2026-04 전국 Shapefile 606,145,264바이트·게시 MD5 확인. RoadLink 52격자 중 TQ 1격자의 452,943개 링크를 OSTN15 변환해 20,851개 타일 팩으로 만들고 기본 세계 모드에서 캡처. 나머지 51격자, 격자 중복 제거와 독립 정확도 검증은 미완료. 북아일랜드 원천은 별도로 필요. |

//! Split a large approved NRN archive at a source-derived z10 tile column.
...
async fn main() -> Result<(), DynError> {
    ...
    if args.len() != 4 {
        return Err("usage: shard_ca_nrn_pmtiles FULL.toml FULL.pmtiles OUTPUT_PREFIX".into());
    }
    ...
    if approved.source.len() != 1 || approved.source[0].adapter != "ca-nrn-roadseg" {
        return Err("expected one approved Canadian NRN source".into());
    }
    ...
    let credit = approved.source[0]
        .attribution_text
        .as_deref()
        .ok_or("missing required Canadian source credit")?;
    if !metadata.contains(credit) {
        return Err("source archive omits required credit".into());
    }
    ...
    west_manifest.proof_bbox_wgs84 = [west, south, split_lon, north];
    let mut east_manifest = raw;
    east_manifest.proof_region.push_str("-east");
    east_manifest.proof_bbox_wgs84 = [split_lon, south, east, north];
    ...
    println!(
        "split_lon={split_lon} z10_split_x={split_x} west_tiles={} east_tiles={} byte_identical_tiles={verified} west_bytes={} east_bytes={}",
        counts[0],
        counts[1],

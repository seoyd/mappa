...
기본 세계지도의 저배율은 Natural Earth 기반이다. 고배율에는 기존 지역 패키지 44개, 영국 Ordnance Survey Open Roads의 GB 격자 패키지 52개, 캐나다 NRN 13개 지역 도로 팩 15개를 필요할 때 연다. GB 팩은 일반화된 도로 선형이며 북아일랜드를 포함하지 않는다. 나주 자체 GeoDB 시안은 S16 `NO_GO` 상태이므로 `canonical-proof`를 명시해야 연다.
